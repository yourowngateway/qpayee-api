  ...
  describe('formatMessageForPublicChannel', () => {
    it(`${constants.COLLECTIVE_TRANSACTION_CREATED} donation`, () => {
      ...
      expect(message).to.equal(
        'New financial contribution: someone gave USD 10.42 to <https://qpayee.com/pubquiz|Pub quiz>',
      );
    });
    ...
    it(`${constants.COLLECTIVE_TRANSACTION_CREATED} expense`, () => {
      ...
      expect(message).to.equal(
        'New transaction for paid expense "pizza" (USD 12.98) on <https://qpayee.com/pubquiz|Pub quiz>',
      );
    });
    ...
    it(`${constants.COLLECTIVE_TRANSACTION_CREATED} refund`, () => {
      ...
      expect(message).to.equal(
        'A transaction (USD 12.98) on <https://qpayee.com/pubquiz|Pub quiz> was refunded: Refund of test contribution',
      );
    });
    ...
    it(`${constants.COLLECTIVE_EXPENSE_PAID} expense paid`, () => {
      ...
      expect(message).to.equal(
        "Expense paid on <https://qpayee.com/pubquiz|Pub quiz>: USD 12.98 for '<http://localhost:3000/pubquiz/expenses/42|pizza>'",
      );
    });

    it(constants.SUBSCRIPTION_CONFIRMED, () => {
      const { message } = activitiesLib.formatMessageForPublicChannel(activitiesData[15], 'slack');
      expect(message).to.equal('New subscription confirmed: EUR 12.34 from someone to <https://qpayee.com/blah|Blah>');
    });
